"""
Guards against Python-side regressions in ClinicalFeatureEngineer — the
legacy FastAPI backend's own feature engineering. This is no longer a
Python/JS parity check (see export_golden_vectors.py's module docstring):
since the Round 6 TypeScript migration (docs/DECISIONS.md §33), the
authoritative implementation is packages/clinical-core/src/features.ts,
which is not mirrored here. This test and its fixture exist only as long
as backend/app/ does — slated for removal together at the deferred FastAPI
cutover.

Every fixture input carries an explicit _reference_month (set by
train_classifier.py::generate_patient), so seasonal_risk no longer depends
on real wall-clock time for these vectors — but this test still freezes
datetime.now() (matching export_golden_vectors.py) as a defensive fallback
for any future contextual feature that reads it directly, and to guard
against a regression that silently drops _reference_month from the fixture.
"""
import json
...
def test_feature_engineering_matches_golden_vectors():
    ...
    assert not mismatches, (
        ...
        "regenerate the fixture: cd tools/training && python export_golden_vectors.py\n"
        + "\n".join(mismatches[:20])
    )
