...
mode (synthetic patients, heuristic reference) that PROVES THE MACHINERY only —
...
Run:
    ...
    PYTHONPATH=. python scripts/evaluate_on_real.py --self-test
    PYTHONPATH=. python scripts/evaluate_on_real.py --csv data/mimic_ed.csv \
...
def self_test(n=8000):
    ...
    The reference here is the heuristic the model was trained to reproduce, so
    the numbers are meaningless as clinical validation (they will look great).
    Its only purpose is to show the report runs and the metrics are sane."""
    ...
    formdatas, y_ref = [], []
    for _ in range(n):
        p = tc.generate_patient(np.random.choice(sevs, p=w),
                                pediatric=np.random.random() < 0.22)
        formdatas.append(p)
        y_ref.append(tc.assign_triage_label(p))
    print("\n" + "!" * 74)
    print("!! SELF-TEST MODE — reference = the training heuristic. This validates")
    print("!! the HARNESS, not the model. These numbers are NOT clinical validation.")
    print("!" * 74 + "\n")
    return formdatas, np.array(y_ref, dtype=int)
...
def main():
    ...
    if args.self_test:
        fds, y = self_test()
        evaluate(fds, y, "SELF-TEST (synthetic, heuristic reference)")
    elif args.csv:
