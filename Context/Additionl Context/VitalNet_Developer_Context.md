...
Temperature: 0.1.
Timeout: 8 seconds. If timeout or 429: return a graceful error state — triage badge still displays, briefing panel shows "Briefing unavailable — triage classification intact."
...
class CaseRecord(Base):
    ...
    # ASHA identity
    asha_id = Column(String, nullable=False)
    ...
    complaint_duration = Column(String)
    ...
    # Symptoms
    symptoms_json = Column(Text)  # JSON array stored as string
    observations = Column(Text, nullable=True)

    # AI outputs
    triage_level = Column(String)  # EMERGENCY | URGENT | ROUTINE
    confidence_score = Column(Float)
    risk_driver = Column(Text)
    briefing_json = Column(Text)  # Full LLM output as JSON string
...
**vite.config.js** — proxy in dev, env var in production:
```javascript
export default {
  server: {
    proxy: {
