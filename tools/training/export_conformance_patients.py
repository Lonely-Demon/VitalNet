...
def main():
    ...
    with patch("app.ml.clinical_features.datetime", _FrozenDateTime), \
        ...
        for i in range(n):
            ...
            # All records are fully synthetic (generated by generate_patient())
            # — no real patient data is ever present in this script. This
            # JSONL file is a dev/training artefact written to a gitignored
            # conformance fixture directory. Suppression uses the current
            # CodeQL inline syntax (codeql[query-id]), not lgtm.com syntax.
